...
response. The tight-window test uses a divergent provider and asserts at least one response reports
`CompactionLevel.High` and at least one reports `MaterialDropped`.
