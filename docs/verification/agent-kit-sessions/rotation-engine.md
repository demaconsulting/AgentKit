...
whether the result will fit, and the engine rotates once at the level it is handed. Oversized
summarizer input is chunked by whole turns before it is consolidated.

Pathological cases are first-class evidence. Tests use a blank summarizer answer, a blank answer from
one chunk of a split consolidation, and a tail already shorter than its configured maximum, to prove
normalization, retention and progress under conditions that must not regress.
...
summarizer whole, splits one turn across two summarizer calls, consolidates nothing while reporting a
...
#### AgentKitSessions-RotationEngine-ChunksOversizedMaterial: Oversized Material Is Chunked
...
#### AgentKitSessions-RotationEngine-KeepsTurnsIndivisible: No Turn Is Split Across Two Calls

**Test**: `RotationEngine_Rotate_OversizedMaterial_IsChunked`

The same chunked rotation is inspected call by call: for every turn in the material, the test asserts
that a call carrying that turn's user entry also carries its answer. A turn is therefore placed whole
in one call or another and never across two, which is what keeps a tool result from reaching a
summarizer without the call it answers. Flattening the material to entries before grouping — the
behavior this replaced — fails the assertion.
...
needed: grouping is what makes a boundary turn-granular, and returning turns is what keeps the
consolidation chunking from splitting one across summarizer calls.

#### AgentKitSessions-RotationEngine-MovesAtLeastOneTurn: A Rotation Always Moves Something
...
tokens, while the verbatim tail is held by a count of turns, so a provider counting well above this
library's estimate reaches its threshold while the tail is still shorter than its configured maximum.
...
- `RotationEngine_Rotate_OneBlankChunk_KeepsTheWholeMaterial`
...
The chunked test is the same defect one level down, and it is the reason a blank record from one
chunk fails the whole consolidation. The second of three chunks is made to come back blank; the test
asserts no slot was written, that every turn is still in the tail, and that the outcome reports
dropped material. Filtering the blank record out and combining the survivors — the behavior this
replaced — would leave that chunk's span of history nowhere, under a result that looked like an
ordinary success.
