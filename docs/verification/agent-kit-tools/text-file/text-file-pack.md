...
calls three factories in this same subsystem, and passes on the access policy it was given. What
must be verified is what a composing application can observe — the prefix it claims, the capability
it requires, the tools it produces and their order, and that the policy supplied is the one
governing them.

The last of those is asserted **behaviorally** rather than by reference comparison: the scenario
creates the pack's tools from a policy rooted at one location, then reads one permitted path and one
...
Unit tests reside in `TextFile/TextFilePackTests.cs`, with the shared reparse-point fixture in
...
requirement other than none, a tool count or order other than the declared one, a null element, a
tool outside the family prefix, an accepted null policy, and a created tool that does not observe
the supplied policy each constitute a failure.

#### Test Scenarios

##### AgentKitTools-TextFile-Pack-FamilyPrefix: The Constant Is the Family Name

**Test**: `TextFilePack_FamilyPrefix_Constant_IsTextFile`

Asserts the published constant is `text_file` — the prefix every tool name in the family is
qualified by.

##### AgentKitTools-TextFile-Pack-FamilyPrefix: The Contract Reports the Declared Constant
...
##### AgentKitTools-TextFile-Pack-NoCapabilityRequired: The Pack Requires No Host Capability

**Test**: `TextFilePack_RequiredCapabilities_Pack_RequiresNoHostCapability`

Normal operation: requiring nothing is what makes the family available to every host rather than
gated behind a declaration an operator would have to know to make.

##### AgentKitTools-TextFile-Pack-RegistersThreeTools: The Read, Write and List Tools Are Created

**Test**: `TextFilePack_CreateTools_Policy_CreatesTheReadWriteAndListTools`

Asserts the count and each name by position, pinning the order because the order a model sees its
tools in is observable.

##### AgentKitTools-TextFile-Pack-RegistersThreeTools: No Null Tool Is Returned
...
##### AgentKitTools-TextFile-Pack-ToolsCarryFamilyPrefix: Every Tool Carries the Family Prefix
...
##### AgentKitTools-TextFile-Pack-RequiresPolicy: A Null Policy Throws
...
##### AgentKitTools-TextFile-Pack-PolicyGovernsCreatedTools: The Supplied Policy Governs the Tools
