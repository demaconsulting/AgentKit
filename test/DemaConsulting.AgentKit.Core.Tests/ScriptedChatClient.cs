...
/// <remarks>
///     <para>
///     The behavior under verification is a rewrite of the message sequence, so the only thing a
///     test needs from a client is a faithful record of what it received. A real provider would
///     add network access, credentials and non-determinism without contributing anything to the
///     assertion, and the asymmetry that motivates the decorator cannot be observed from inside
///     this process in any case — it happens at a provider's wire format.
///     </para>
///     <para>
///     Written by hand rather than substituted, because the streaming member returns an
///     asynchronous sequence that a substitute would have to be taught to produce anyway.
///     </para>
///     <para>
