...
public sealed class InMemoryProviderSession : IProviderSession
{
        ...
        int windowTokens)
    {
        ...
        FixedOverheadTokens = CountTokens(seed.Instructions)
            + seed.Tools.Sum(tool => CountTokens(tool.Name) + CountTokens(tool.Description) + PerItemTokens);
    }

    /// <summary>
    ///     The tokens charged for one item beyond the characters it carries, standing in for the
    ///     framing a real provider wraps around a message or a declaration.
    /// </summary>
    private const int PerItemTokens = 4;

    /// <summary>
    ///     Counts the tokens a run of text occupies, at four characters to the token.
    /// </summary>
    /// <remarks>
    ///     An adapter answers for its own provider, so an adapter whose provider publishes no
    ///     figures counts for itself. This one stands in for a provider that does publish them, and
    ///     counts the same way the ratio a rule of thumb would - which is all a stand-in needs, and
    ///     is deliberately the adapter's own business rather than something the engine supplies.
    /// </remarks>
    /// <param name="text">The text to count. May be <see langword="null"/>.</param>
    ...
    public ContextUsage CurrentUsage
    {
        get
        {
            ...
            {
                conversation += CountTokens(entry.Text) + PerItemTokens;
            }
