...
public sealed class CompactingAgentSession : IAgentSession
{
    ...
    ///     reads the provider's usage, which is adapter code. So the construction is guarded and the
    ///     provider session is released before the failure surfaces. Where that release also fails,
    ///     the provider is left holding a session this library can no longer reach and will reclaim
    ///     it when the session expires - the caller is told what went wrong rather than handed a
    ///     handle to retry a disposal that has just failed.
    ///     </para>
    /// </remarks>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch
        {
            ...
            //
            // The release is attempted and its outcome deliberately not acted on. Where it fails the
            // provider is left holding a session this library can no longer reach, and will reclaim
            // it when the session expires; handing the caller a handle to retry a disposal that just
            // failed asks them to write recovery code for two simultaneous adapter defects, with a
            // remedy unlikely to work. The creation failure is what they can act on, so it is what
            // they are given, unwrapped and where the adapter wrote it.
            await TryReleaseUnownedAsync(provider).ConfigureAwait(false);
            throw;
        }
    }
    ...
    private static (bool Dropped, ContextLayout Layout) DropOldest(ContextLayout layout)
    {
        for (var index = ContextLayout.TierCount - 1; index >= 0; index--)
        {
            ...
            return (true, ContextLayout.WithTiers(layout.Tail, tiers));
        }
