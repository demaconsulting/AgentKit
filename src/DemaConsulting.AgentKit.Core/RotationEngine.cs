...
/// <summary>
///     What one rotation produced: the new layout, how much summarizer work it cost, and whether it
///     had to discard material outright to make room.
/// </summary>
/// <remarks>
...
internal sealed class RotationOutcome
{
    ...
    ///     Whether the rotation displaced a tier's oldest slot outright because the tier was full
    ///     and could not be emptied into a coarser one.
    /// </param>
    ...
    /// </summary>
    /// <remarks>
    ///     True when a tier was full, could not be emptied into a coarser one, and its oldest slot
    ///     was displaced to make room for the arriving one — the coarsest tier doing so as a ring by
    ///     design, a finer tier only when its cascade consolidation came back blank. It is the
    ///     honest signal that compaction bought nothing, and is deliberately distinct from a
    ///     consolidation that simply could not be made: that leaves the material where it is, which
    ///     is not a loss.
    /// </remarks>
...
///     </para>
///     <para>
///     <b>Nothing here measures anything.</b> The context a rotation builds has not been sent, so
///     there is no figure to judge it by: the only token figure this library ever sees is the one a
///     provider reports after a turn, and it reaches the session, not the engine. A rotation moves
///     material and reports what it did.
///     </para>
///     <para>
///     <b>A rotation always moves something.</b> The tail keeps at most what the level asks for and
///     at most one turn fewer than it holds, so a session whose provider reaches the rotation
///     threshold while the tail is still shorter than its configured maximum cannot produce a
...
internal static class RotationEngine
{
    ...
    /// </summary>
    /// <remarks>
    ///     Nothing here measures anything. A rotation moves material and reports what it did;
    ///     deciding how hard to compact, and whether to discard a slot outright, belongs to the
    ///     session, which is the only place that knows how quickly the window filled.
    /// </remarks>
    ...
    /// <returns>The aged layout, the consolidation count, and whether material was dropped.</returns>
    ...
    /// </summary>
    /// <param name="layout">The layout to age.</param>
    /// <param name="summarizer">The out-of-session summarizer.</param>
    /// <param name="level">The compaction level to rotate at.</param>
    /// <param name="verbatimTurns">The configured maximum verbatim tail length.</param>
    /// <param name="cancellationToken">Cancels the consolidation.</param>
    /// <returns>
    ///     The rotated layout at this level, how many consolidations it cost, and whether a tier
    ///     displaced its oldest slot.
    /// </returns>
        ...
        CancellationToken cancellationToken)
    {
        ...
        // can reach its rotation threshold while the tail is still shorter than its configured
        // maximum. Keeping the level's figure blindly would then leave nothing older to consolidate,
        // produce a layout identical to the one handed in, and send the session back to a provider
        // it has already been told is full. So the tail keeps at most what the level asks for and at
        // most one turn fewer than it holds, whichever is smaller.
        var keep = Math.Min(
        ...
        if (keep < 0)
        {
            // No turns at all: the tail is empty, so there is nothing to move. A tail holding one
            // turn keeps none of it and reaches the split below instead.
    ...
    private sealed class RotationState
    {
        ...
        /// </summary>
        /// <remarks>
        ///     Set when a full tier had nowhere to put an arriving slot and displaced its oldest:
        ///     the coarsest tier doing so as the ring of rule four, or a finer tier whose cascade
        ///     consolidation came back blank, so it could not be cleared into a record that was
        ///     never written. Reported through
        ...
        /// </summary>
        /// <remarks>
        ///     Each turn is rendered as a single piece and the pieces are sent together in one call,
        ///     so a turn can never be split across summarizer calls. That is what makes turn-granular
        ///     boundaries real: a tool result can never reach a summarizer without the tool call it
        ///     answers.
        /// </remarks>
