...
/// <summary>
///     The <c>file_move</c> tool: moves one file from a source the access policy permits the agent
///     to write to a destination the policy permits it to write.
/// </summary>
/// <remarks>
///     <para>
///     <b>Both endpoints are judged by the write decision, because a move changes both.</b> A move
///     removes the source and creates the destination, so unlike a copy it needs write permission at
///     the source as well as the destination. A file an agent may read but not write cannot be moved
///     out of its location, which is what keeps a read-only grant genuinely read-only: it can be
///     copied elsewhere, but the original cannot be taken away.
///     </para>
///     <para>
///     <b>An existing destination is refused unless the request explicitly permits the overwrite.</b>
///     Silently replacing a file an agent did not mean to touch is an irreversible side effect this
///     library exists to make impossible to express by accident. The overwrite flag defaults to
///     refusing, and the refusal names the flag so the model learns how to proceed when the
...
///     workspace: source control, which is why an agent should be run in a repository.
///     </para>
///     <para>
...
public static class FileMoveTool
{
    ...
    /// </summary>
    public const string ToolName = "file_move";
    ...
    private const string SourceRequired =
        "A source path is required. Supply the path of the file to move, relative to the workspace "
    ...
    private const string DestinationRequired =
        "A destination path is required. Supply the path to move the file to, relative to the "
        + "workspace root, for example 'archive/notes.txt'.";

    /// <summary>
    ///     The refusal used when the source names a directory rather than a file.
    /// </summary>
    private const string SourceIsDirectory =
        "The source path is a directory, not a file. This tool moves a single file; name the file "
        + "to move.";
    ...
    /// </summary>
    private const string MoveFailed = "The file could not be moved.";

    /// <summary>
    ///     Creates the <c>file_move</c> tool governed by an access policy.
    /// </summary>
    /// <remarks>
    ...
    internal static AIFunction Create(PathPolicy policy)
    {
        ...
        var move = (
                [Description(
                    "The path of the file to move, relative to the workspace root, "
                    + "for example 'notes.txt'.")]
                string? source = null,
                [Description(
                    "The path to move the file to, relative to the workspace root, "
                    + "for example 'archive/notes.txt'.")]
            ...
            Move(policy, source, destination, overwrite);

        return GuardedToolFactory.Create(move, ToolName, ToolDescription);
    }

    /// <summary>
    ///     Moves one file, refusing rather than throwing whenever the request cannot be honored.
    /// </summary>
    /// <remarks>
    ...
    /// <param name="source">The path the model wishes to move, or null when it supplied none.</param>
        ...
        bool overwrite)
    {
        ...
        // A directory source has no single file to move; the tool never recurses.
        ...
        // Writing over a directory is not a meaningful file move.
        ...
        try
        {
            ...
            System.IO.File.Move(realSource, realDestination, overwrite);

            return ToolResult.Text("Moved the file.");
        }
