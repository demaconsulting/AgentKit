...
### `markdown_sections` — a path-taking tool (the `markdown` pack)

Lists the section headings of a Markdown file together with their 1-based line numbers. It is a
worked example of an author writing a tool that touches the file system, and it does so the same way
a shipped tool does:
...
- **It routes every path through `PathPolicy` for containment.** A path outside the granted
  ...
  containment refusal, `TargetNotFound` for a missing file, `ResourceTooLarge` for a file or listing
  over the policy's ceilings, `InvalidRequest` for a directory), never thrown.
- **It returns a structured result** via `ToolResult.Structured`, so the headings arrive as
  machine-readable fields — `path`, `sectionCount`, and a `sections` array of `{ level, title, line }`
  — rather than as prose a model would have to re-parse.
- **It reports file locations in the same path dialect the shipped tools use.** This is a specific
  ...
  can search and whether relative addressing is meaningful, and `PathPolicy.EmitRelative` to mirror
...
contrast to the Markdown tool: it demonstrates that `GuardedToolFactory` is the construction path for
...
`sample.md` with several headings.
...
- **A structured section listing.** "List the sections of sample.md with their line numbers." The
  agent calls `markdown_sections` with `sample.md`; the result is a structured object naming each
  heading's level, title, and line, with the file reported by its bare relative name because it lies
  inside the granted workspace.
...
- **Containment.** "List the sections of ../outside.md." The `markdown_sections` tool routes the
  path through `PathPolicy`, which refuses it, and returns `Denied (PathNotPermitted)`. The sibling
  file `outside.md` exists precisely so this refusal is a real containment decision.
- **Discovery.** "What locations can the markdown tool search?" A no-argument `markdown_sections`
  call returns a structured discovery result listing the searchable roots and whether relative
  addressing applies — built from the same `PathPolicy` helpers the shipped tools use.
- **Composition with a shipped pack.** "List the files in the workspace, then list the sections of
  each Markdown file you find." The agent uses the shipped `text_file_list` and the custom
  `markdown_sections` together, proving the packs coexist.
